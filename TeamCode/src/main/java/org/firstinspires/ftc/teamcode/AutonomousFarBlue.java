...
@Autonomous
public class AutonomousFarBlue extends OpMode {
...
public void init() {
    ...
    DrivetoBalls = follower.pathBuilder()
            ...
            .setLinearHeadingInterpolation(startPose.getHeading(),BallPose.getHeading() ) // Drehung während der Fahrt
            .build();
    ...
    follower.update();
        ...
        if ( follower.getVelocity().getMagnitude() < 0.1) {
            // Sanfter Reset der Position, um Sprünge zu vermeiden
            follower.setPose(calculatingMethods.CalculateCamPos(aprilTagWebcam.getLastDetection(),dataStorage.BlueGoal));
        }
        switch (currentState) {
            case ReadAprilTag:
                ...
                if (stateTimer.seconds() > 2 || read == true) {
                    //turret.setTargetAngle(CalculatingMethods.CalculateTurretGoalAngel(blueGoal);
                    stateTimer.reset();
            ...
            case Shoot:
                if (Math.abs(calculatingMethods.CalculateTurretGoalAngle(dataStorage.BlueGoal)-calculatingMethods.TurretAngle())<Math.toRadians(3)|| stateTimer.seconds()>1){
            ...
            case CollectBalls:
                ...
                if (!follower.isBusy()||stateTimer.seconds()>2){
                    stateTimer.reset();
                currentState=States.Drive_to_Shoot;
                    PathStarted=false;
                }
